...
import {
	convertEventToVote
} from '../../../src/format/data-format-details'
import * as staking_modules from '../../../src/staking/staking'

describe('convertEventToVote', () => {
	let getAllStakingValue: sinon.SinonStub<[devInstance: Contract, propertyGroupInstance: Contract, user: string, toBlock: number], Promise<BigNumber>>
	before(() => {
	...
	it('Event data will be converted to voting data.', async () => {
		...
		const voteData = await convertEventToVote(args as any, {} as any, {} as any, 100)
